...
namespace RFID_Read
{
    public partial class Form1 : Form
    {
        ...
        private void updateTagData(TagReadData tag) 
        {
            ...
            {
                curEPC = EBdData;
                meb = InStock.getMemDetailByID(EBdData);
                meb.EPCStr = EBdData;
        ...
        private void refreshDataView() // binding source change
        {
            ...
            foreach (goodDetail goodTemp in goodsDetailList)
            {
                ...
                InStock.updateMemCostDetail(new costInfo(meb.ID, goodTemp.ID, DateTime.Now.ToString(), InStock.getBillNumber(),Convert.ToInt32(detail[1]), Convert.ToDouble(detail[0]) / Convert.ToInt32(detail[1]), Convert.ToDouble(detail[0])));
            }
